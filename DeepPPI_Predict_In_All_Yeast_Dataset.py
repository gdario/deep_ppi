...
def get_sep_model():
    input_1 = Input(shape=(5, ), name='Protein_a')
    ...
    protein_input1 = Dropout(0.0)(protein_input1)
    protein_input1 = Dense(
        256,
        activation='relu',
        init='he_uniform',
    ...
    protein_input1 = Dense(
        ...
        W_regularizer=l2(0.01))(protein_input1)
    protein_input1 = Dropout(0.0)(protein_input1)
    input_2 = Input(shape=(5, ), name='Protein_b')
    ...
    protein_input2 = Dropout(0.0)(protein_input2)
    protein_input2 = Dense(
        256,
        activation='relu',
        init='he_uniform',
    ...
    merged_vector = merge(
        ...
        name='merge_pro_A_B')
    output = Dense(
        128, activation='relu', init='he_uniform',
...
def train_sep_model(protein):
    X = protein.iloc[:, :5].values.astype("float")
    y = protein.iloc[:, 5].values.astype("int")
    #shuffle data
    np.random.seed(1)
...
yeast_gold_protein = pd.read_csv(
    'dataset/Yeast/yeast_gold.tab', sep='	').iloc[2:, 2:]
yeast_silver_protein = pd.read_csv(
    'dataset/Yeast/yeast_silver.tab', sep='	').iloc[2:, 2:]

gold_sep_model = train_sep_model(yeast_gold_protein)

silver_sep_model = train_sep_model(yeast_silver_protein)

test_set = pd.read_csv('dataset/Yeast/yeast_all.tab', sep='	').iloc[2:, 2:]
X_test = test_set.iloc[:, :5].values.astype("float")
y_test = test_set.iloc[:, 5].values.astype("int")
print('train on yeast gold test yeast all')
pred_sep_model(gold_sep_model, X_test, y_test)

print('train on yeast silver test yeast all')
